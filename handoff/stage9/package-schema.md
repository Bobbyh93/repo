...
Version: 1.1 (2026-09-05) — adds §11 traceability and §12 improvement loop
Consumed by: `scripts/generate_lesson_package.py` (Stage 9)
...
From `runtime_config.layout.archetype_set`. The generator has a layout for each; unknown archetypes fall back to `content` and are logged as a **major** defect.
...
|---|---|
| `chapter_map` | `{"lane": "string", "nodes": ["string"]}` — one per concept lane, ordered foundation → bedside |
| `concept_cards` | `{"heading": "string", "body": "string", "cjm": "string"}` — 2 to 6 cards |
| `checkpoint_mcq`, `capstone_mcq` | `{"stem": "string", "options": ["A. …"], "correct": "A", "rationale": "string"}` — exactly one element |
| `mini_case`, `opening_case` | `{"presentation": "string", "cues": ["string"], "prompt": "string"}` — exactly one element |
| `urgency_sort` | `{"items": ["string"], "correct_order": [0,2,1]}` — exactly one element |
| `match_activity` | `{"left": ["string"], "right": ["string"], "pairs": [[0,1]]}` — exactly one element |
...
```
python scripts/generate_lesson_package.py --spec lesson_spec.json --outdir OUTDIR
python scripts/generate_lesson_package.py --demo --outdir OUTDIR
```
